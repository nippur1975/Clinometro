...
def main():
    ...
    global app_running, dimensiones, request_restore_display # Necesario para el bucle principal y restauración
    ...
    while app_running:
        # Lógica para restaurar la ventana
        if request_restore_display:
            if not pygame.display.get_init():
                pygame.display.init()
                screen = pygame.display.set_mode(dimensiones)
                pygame.display.set_caption(TEXTOS[IDIOMA]["titulo_ventana"])
                # Recargar assets visuales si es necesario
                try:
                    imagen_fondo_original = pygame.image.load(resource_path("mar.jpg"))
                    imagen_fondo_escalada = pygame.transform.scale(imagen_fondo_original, dimensiones).convert()
                except Exception as e:
                    print(f"ERROR: Recargando imagen_fondo_escalada: {e}")
                    imagen_fondo_escalada = None
            request_restore_display = False
        
        # Si el display no está activo, estamos en modo bandeja
        if not pygame.display.get_init():
            pygame.time.wait(100) # Esperar para no consumir CPU
        else:
            # --- TODA LA LÓGICA DE RENDERIZADO VA AQUÍ DENTRO ---
        ...
        reloj.tick(30) # Limitar FPS para no consumir CPU innecesariamente
    
    # Limpieza al salir del bucle principal
    print("Saliendo del bucle principal, limpiando recursos...")
    if ser and ser.is_open:
        ser.close()
    ...
    if tray_icon and tray_icon.visible:
        tray_icon.stop()
        print("Icono de bandeja detenido.")
        
    pygame.quit()
    print("Pygame quitado. Saliendo del programa.")
...
def quit_app(icon, _item):
    global app_running
    print("DEBUG: quit_app CALLED BY PSTRAY")
    app_running = False
    if icon:
        icon.stop()
    # No llamar a pygame.quit() o sys.exit() desde este hilo, 
    # el bucle principal se encargará de ello.

def show_window(icon, _item):
    global request_restore_display
    print("DEBUG: show_window CALLED BY PSTRAY")
    request_restore_display = True
    if icon:
        icon.stop()

def hide_window():
    """Oculta la ventana de Pygame y muestra el icono de la bandeja."""
    global tray_icon, dimensiones
    pygame.display.quit() # Desinicializa el módulo de display
    ...
    try:
        ...
        # Crear una imagen simple por defecto si falla la carga
        image = Image.new("RGB", (64, 64), color = 'blue')

    menu = (item('Mostrar', show_window, default=True), item('Salir', quit_app))
    
    # Crear y correr el icono en un hilo separado para no bloquear
    def run_tray_icon():
        global tray_icon
    ...
    tray_thread.start()
    print("DEBUG: hide_window - Icono de bandeja corriendo en hilo separado.")
