...
def main():
    ...
    global IDIOMA, screen, imagen_fondo_escalada, pitch_image_base_grande, roll_image_base_grande
    global request_restore_display, app_running, TEXTOS, AZUL, resource_path, dimensiones
    global lado_pitch_deseado_grande, lado_img_deseado_grande, radio_circulo_img # Asegúrate que estas estén si son necesarias aquí
    # ... (otras globales que uses en main) ...




    print("DEBUG: main() - Initializing Pygame and basic display", flush=True)
    pygame.init()
 ...
 # Carga inicial de imágenes - simplificado para depurar
    try:
        print("DEBUG: main() - Initial load of imagen_fondo_escalada...", flush=True)
        imagen_fondo_original = pygame.image.load(resource_path("mar.jpg"))
        imagen_fondo_escalada = pygame.transform.scale(imagen_fondo_original, dimensiones).convert()
        print(f"DEBUG: main() - imagen_fondo_escalada loaded. Valid: {imagen_fondo_escalada is not None}", flush=True)
    except Exception as e:
        print(f"ERROR FATAL: Carga inicial de imagen_fondo_escalada falló: {e}", flush=True)
        imagen_fondo_escalada = None 
        # Considera salir si esto es crítico: pygame.quit(); sys.exit()
    
    # Simplifica la carga de otras imágenes para depurar, asegúrate que las variables estén definidas
    radio_circulo_img = 78 * 2 # Ejemplo, asegúrate que esté definida
    ...
    try:
        ...
        print(f"ERROR: Carga inicial de pitch_image_base_grande falló: {e}", flush=True)
    ...
    try:
        ...
        print(f"ERROR: Carga inicial de roll_image_base_grande falló: {e}", flush=True)
        roll_image_base_grande = None

    # ... (resto de tus inicializaciones: cargar_configuracion_serial, etc.)
    cargar_configuracion_serial() # Ejemplo
    cargar_configuracion_alarma() # Ejemplo
    reset_ui_data() # Ejemplo
    init_csv() # Ejemplo
    init_alarm_csv() # Ejemplo

    print("DEBUG: main() - Entrando al bucle principal while app_running", flush=True)
    while app_running:
        print(f"DEBUG [Loop Top]: app_running={app_running}, request_restore_display={request_restore_display}, pygame.display.get_init()={pygame.display.get_init()}", flush=True)

        if request_restore_display:
            print("DEBUG [Restore Block]: request_restore_display is True. Reinitializing...", flush=True)
            if not pygame.get_init(): # Si Pygame fue quitado completamente
                print("DEBUG [Restore Block]: Pygame no estaba inicializado. Llamando pygame.init()", flush=True)
                pygame.init()
            elif not pygame.display.get_init(): # Si solo el display fue quitado
                print("DEBUG [Restore Block]: Display no estaba inicializado. Llamando pygame.display.init()", flush=True)
                pygame.display.init()
            else:
                print("DEBUG [Restore Block]: Pygame y Display ya estaban inicializados.", flush=True)
            
            screen = pygame.display.set_mode(dimensiones) 
            pygame.display.set_caption(TEXTOS[IDIOMA]["titulo_ventana"])
            print("DEBUG [Restore Block]: Display mode y caption seteados.", flush=True)

            try:
                imagen_fondo_original = pygame.image.load(resource_path("mar.jpg"))
                imagen_fondo_escalada = pygame.transform.scale(imagen_fondo_original, dimensiones).convert()
                print(f"DEBUG [Restore Block]: imagen_fondo_escalada recargada. Es None: {imagen_fondo_escalada is None}. Size: {imagen_fondo_escalada.get_size() if imagen_fondo_escalada else 'N/A'}", flush=True)
            except Exception as e:
                print(f"ERROR [Restore Block]: Recargando imagen_fondo_escalada: {e}", flush=True)
                imagen_fondo_escalada = None
            # (Recarga similar para pitch_image_base_grande y roll_image_base_grande con prints)
            # ...

            request_restore_display = False
            print("DEBUG [Restore Block]: request_restore_display = False.", flush=True)
            
            if pygame.display.get_init():
                print("DEBUG [Restore Block]: Display OK para flip inicial.", flush=True)
                ...
                print("ERROR [Restore Block]: Display NO INICIALIZADO para flip inicial.", flush=True)
            
            print("DEBUG [Restore Block]: FIN, ejecutando continue.", flush=True)
            continue 

        if not pygame.display.get_init():
            print(f"DEBUG [Loop Guard]: Display not init. Waiting. app_running: {app_running}", flush=True)
            pygame.time.wait(100)
            continue
        
        # --- A PARTIR DE AQUÍ, EL DISPLAY DEBERÍA ESTAR INICIALIZADO ---
        # print("DEBUG [Main Flow]: Display OK. Procesando eventos y renderizando.", flush=True) # Muy verboso

        mouse_pos = pygame.mouse.get_pos() 
        
        for evento in pygame.event.get():
            if evento.type == pygame.QUIT:
                print("DEBUG [Event Loop]: pygame.QUIT detectado. Llamando a hide_window().", flush=True)
                hide_window()
            # ... (tu manejo de otros eventos: MOUSEBUTTONDOWN, KEYDOWN, etc.)

        # ... (tu lógica de actualización de datos: NMEA, alarmas, etc.)

        # --- SECCIÓN DE RENDERIZADO --- 
        # print(f"DEBUG [Render]: Iniciando renderizado. imagen_fondo_escalada es None: {imagen_fondo_escalada is None}", flush=True)
        if imagen_fondo_escalada:
            try:
                screen.blit(imagen_fondo_escalada, (0, 0)) # La línea del error
            except pygame.error as e_blit:
                print(f"ERROR FATAL en screen.blit: {e_blit}", flush=True)
                # Podrías intentar re-inicializar screen aquí como último recurso o salir
                pygame.quit()
                sys.exit()
        else:
            # print("DEBUG [Render]: imagen_fondo_escalada es None, llenando con AZUL.", flush=True)
            screen.fill(AZUL) 
        
        # ... (resto de tu código de renderizado: círculos, texto, etc.)
        pygame.display.flip()
        reloj.tick(60)

# --- Fin del bucle while app_running ---
print("DEBUG: Salida del bucle while app_running. Limpiando...", flush=True)
pygame.quit()
sys.exit() # Asegúrate que sys está importado
    ...
    global request_restore_display # Access the global flag
    
    while app_running: # Usar la variable global app_running
        if request_restore_display:
            print("DEBUG: request_restore_display is True. Reinitializing display...")
            pygame.init() # Reinitialize all Pygame modules
            screen = pygame.display.set_mode(dimensiones) 
            pygame.display.set_caption(TEXTOS[IDIOMA]["titulo_ventana"])

            # Re-initialize critical visual assets
            try:
                imagen_fondo_original = pygame.image.load(resource_path("mar.jpg"))
                imagen_fondo_escalada = pygame.transform.scale(imagen_fondo_original, dimensiones)
                imagen_fondo_escalada = imagen_fondo_escalada.convert()
                print("DEBUG: Background image reloaded and converted.")
            except Exception as e:
                print(f"ERROR: Reloading background image on restore: {e}")
                imagen_fondo_escalada = None
            
            try:
                pitch_image_surface_original_temp = pygame.image.load(resource_path("pitch.png"))
                # lado_pitch_deseado_grande is already defined in main's scope
                pitch_image_base_grande = pygame.transform.smoothscale(pitch_image_surface_original_temp, (lado_pitch_deseado_grande, lado_pitch_deseado_grande)).convert_alpha()
                print("DEBUG: Pitch image reloaded and converted.")
            except Exception as e:
                print(f"ERROR: Reloading pitch image on restore: {e}")
                pitch_image_base_grande = None

            try:
                roll_image_surface_original_temp = pygame.image.load(resource_path("roll.png"))
                # lado_img_deseado_grande is already defined in main's scope
                roll_image_base_grande = pygame.transform.smoothscale(roll_image_surface_original_temp, (lado_img_deseado_grande, lado_img_deseado_grande)).convert_alpha()
                print("DEBUG: Roll image reloaded and converted.")
            except Exception as e:
                print(f"ERROR: Reloading roll image on restore: {e}")
                roll_image_base_grande = None

            request_restore_display = False
            print("DEBUG: request_restore_display now False.")
        ...
        # If display is not initialized (e.g., after hide_window), wait.
        if not pygame.display.get_init():
            pygame.time.wait(100) # Yield/sleep
            # No Pygame event processing here as display is down.
            # Tray icon thread will set app_running or request_restore_display.
            continue # Go to top of the loop to check flags.

        # If we reach here, Pygame display is initialized.
        mouse_pos = pygame.mouse.get_pos()
        ...
        # Manejo de eventos
        for evento in pygame.event.get(): # Process events if display is active
            if evento.type == pygame.QUIT: # User clicked X on Pygame window
                hide_window() # Go to tray
        ...
        # Renderizado
        print(f"DEBUG [Render]: Iniciando ciclo de renderizado. pygame.display.get_init(): {pygame.display.get_init()}", flush=True)
        if pygame.display.get_init(): # Solo intentar acceder a screen si el display está inicializado
            print(f"DEBUG [Render]: Estado de screen. Type: {type(screen)}, Size: {screen.get_size() if screen else 'No screen obj'}, Flags: {screen.get_flags() if screen else 'No screen obj'}", flush=True)
        else:
            print("DEBUG [Render]: pygame.display no inicializado antes de renderizar.", flush=True)

        print(f"DEBUG [Render]: Estado de imagen_fondo_escalada. Es None: {imagen_fondo_escalada is None}", flush=True)
        if imagen_fondo_escalada is not None:
            try:
                img_size = imagen_fondo_escalada.get_size()
                img_flags = imagen_fondo_escalada.get_flags()
                print(f"DEBUG [Render]: imagen_fondo_escalada Type: {type(imagen_fondo_escalada)}, Size: {img_size}, Flags: {img_flags}", flush=True)
                if img_size[0] == 0 or img_size[1] == 0:
                    print("ERROR [Render]: imagen_fondo_escalada tiene dimensión cero. Forzando a None.", flush=True)
                    imagen_fondo_escalada = None # Workaround
            except pygame.error as e:
                print(f"ERROR [Render]: imagen_fondo_escalada inválida al obtener size/flags: {e}. Forzando a None.", flush=True)
                imagen_fondo_escalada = None # Workaround
            except AttributeError: # Si no es una superficie válida, get_size() o get_flags() fallará
                print(f"ERROR [Render]: imagen_fondo_escalada no parece ser un objeto Surface válido. Type: {type(imagen_fondo_escalada)}. Forzando a None.", flush=True)
                imagen_fondo_escalada = None # Workaround
...
tray_icon = None
app_running = True
request_restore_display = False # Flag to signal main loop to restore display
RESTORE_WINDOW_EVENT = pygame.USEREVENT + 1 # Custom event for restoring window
...
def quit_app(_icon, _item):
    global app_running
    print("DEBUG: quit_app CALLED BY PSTRAY", flush=True)
    app_running = False
    if _icon:
        print("DEBUG: quit_app - Stopping icon", flush=True)
        _icon.stop()
    print("DEBUG: quit_app - FINISHED", flush=True)

def show_window(_icon, _item): # _icon es la instancia de pystray.Icon
    global request_restore_display
    print("DEBUG: show_window CALLED BY PSTRAY", flush=True)
    request_restore_display = True
    if _icon:
        print("DEBUG: show_window - Stopping icon", flush=True)
        _icon.stop()
    print("DEBUG: show_window - FINISHED", flush=True)



def hide_window():
    global dimensiones # tray_icon se manejará localmente en esta función
    print("DEBUG: hide_window CALLED", flush=True)
    if pygame.display.get_init():
        pygame.display.quit()
        print("DEBUG: hide_window - pygame.display.quit() called", flush=True)
    else:
        print("DEBUG: hide_window - pygame.display was already not initialized", flush=True)
    
    image_path = resource_path("barco.png")
    try:
        image = Image.open(image_path)
    except Exception as e:
        print(f"ERROR: Could not load tray icon image: {e}", flush=True)
        return # No continuar si no hay icono

    menu_items = (item('Mostrar', show_window, default=True), item('Salir', quit_app))
    # Usar una instancia local para el ícono de la bandeja
    current_tray_icon = pystray.Icon("Lalito_Tray", image, "Lalito Clinómetro", menu_items)
    print("DEBUG: hide_window - pystray.Icon created. Calling run()...", flush=True)
    current_tray_icon.run() # Esto bloquea hasta que se llama a stop() en el ícono
    print("DEBUG: hide_window - current_tray_icon.run() FINISHED.", flush=True)
    # No es necesario gestionar tray_icon global aquí si es local en esta llamada
