...
# Configuración de idioma (agregar con las otras constantes)
...
# Variables para los sonidos de alarma en inglés
sonido_alarma_port_en = None
sonido_alarma_starboard_en = None
sonido_alarma_head_en = None
sonido_alarma_stern_en = None

def cargar_configuracion_serial():
    global IDIOMA # Asegurarse de que IDIOMA se pueda modificar
    try:
        with open(ARCHIVO_CONFIG_SERIAL, 'r') as f:
            config = json.load(f)
        IDIOMA = config.get('idioma', 'es')  # Cargar idioma guardado, default 'es'
        return config.get('puerto', 'COM9'), int(config.get('baudios', 9600))
    except (FileNotFoundError, json.JSONDecodeError, ValueError):
        IDIOMA = 'es' # Default en caso de error o archivo no existente
        return 'COM9', 9600

def guardar_configuracion_serial(puerto, baudios):
    print("DEBUG: Intentando guardar configuración serial e idioma...") 
    config = {
        'puerto': puerto, 
        'baudios': int(baudios),
        'idioma': IDIOMA # Guardar preferencia de idioma
    }
    try:
        with open(ARCHIVO_CONFIG_SERIAL, 'w') as f: json.dump(config, f, indent=4)
        print(f"DEBUG: Configuración serial e idioma guardada: {config}") 
        return True
    except IOError as e:
...
try:
    ...
    # Cargar sonidos en inglés
    # global sonido_alarma_port_en, sonido_alarma_starboard_en, sonido_alarma_head_en, sonido_alarma_stern_en # Eliminada esta línea
    sonido_alarma_port_en = pygame.mixer.Sound(os.path.join(script_dir, "port_alarm.mp3"))
    sonido_alarma_starboard_en = pygame.mixer.Sound(os.path.join(script_dir, "starboard_alarm.mp3"))
    sonido_alarma_head_en = pygame.mixer.Sound(os.path.join(script_dir, "head_alarm.mp3"))
    sonido_alarma_stern_en = pygame.mixer.Sound(os.path.join(script_dir, "stern_alarm.mp3"))
    print("Archivos de alarma en INGLÉS MP3 cargados.")

except pygame.error as e:
    print(f"Error al cargar archivos de sonido MP3: {e}")
    # Asegurarse de que todos los sonidos (español e inglés) sean None si hay un error general,
    # o manejar errores de carga individuales si se prefiere.
    sonido_alarma_babor = None; sonido_alarma_estribor = None; sonido_alarma_sentado = None; sonido_alarma_encabuzado = None
    sonido_alarma_port_en = None; sonido_alarma_starboard_en = None; sonido_alarma_head_en = None; sonido_alarma_stern_en = None
    print("Algunos o todos los archivos de alarma MP3 no se pudieron cargar. La funcionalidad de alarma sonora podría estar afectada.")

ultima_vez_datos_recibidos = pygame.time.get_ticks() 
puerto, baudios = cargar_configuracion_serial() # IDIOMA se carga aquí
cargar_configuracion_alarma() 
ser = None; serial_port_available = True
mostrar_ventana_config_serial = False; mostrar_ventana_acerca_de = False; mostrar_ventana_alarma = False; mostrar_ventana_idioma = False
input_alarma_activo = None 
rect_ventana_alarma = pygame.Rect(250, 100, 380, 230) 
rect_ventana_idioma = pygame.Rect(0, 0, 250, 170) # Ajustar altura para botones y título
rect_boton_es = None
rect_boton_en = None
...
def parse_gga(sentence):
    ...
    try:
        ...
        if len(parts) > 5 and parts[2] and parts[3] and parts[4] and parts[5]:
            ...
            global nmea_data_stale; nmea_data_stale = False
    except: pass
...
def parse_vtg(sentence):
    ...
    try: 
        ...
        if speed_val_str_ui != "N/A": 
            speed_str = f"{speed_val_str_ui} Kt"; ts_speed_float = temp_speed_float; ultima_vez_datos_recibidos = pygame.time.get_ticks()
            global nmea_data_stale; nmea_data_stale = False
...
def reset_ui_data():
    ...
    # No se modifica nmea_data_stale aquí, se maneja fuera.
    ...
    # print("DEBUG: UI data and alarm states reset by reset_ui_data()")

# Inicialización de la pantalla principal
screen = pygame.display.set_mode(dimensiones) 
# El caption se actualiza dentro del bucle principal basado en IDIOMA.
...
nmea_data_stale = False # Inicialización global para el estado de los datos NMEA
print(f"DEBUG: COLOR_INPUT_BORDE justo antes del bucle principal: {globals().get('COLOR_INPUT_BORDE')}")

while not hecho:
    # global nmea_data_stale # Eliminada esta línea que causa SyntaxError
    # nmea_data_stale = False # DEBUG: Forzar reinicialización en cada bucle (quitar después)
    # print(f"DEBUG: mostrar_ventana_idioma before line 561: {mostrar_ventana_idioma}") # Debugging print para NameError anterior, ahora resuelto.
    ...
    if rect_barra_herramientas.collidepoint(mouse_pos):
    ...
    for evento in pygame.event.get():  
        ...
        if evento.type == pygame.KEYDOWN:
            if mostrar_ventana_config_serial: 
                ...
                if evento.key == pygame.K_ESCAPE: mostrar_ventana_idioma = False
            elif mostrar_ventana_acerca_de: 
    ...
    # La variable 'opciones_menu_barra' se define al inicio del bucle 'while not hecho'.
    # Se elimina la definición local de 'opciones_menu_barra_actualizadas'.

    if toolbar_visible: # Only draw toolbar if visible
        ...
        for opcion_texto in opciones_menu_barra: # Corregido para usar la variable correcta
    ...
    text_surface_titulo_latlon = font.render(TEXTOS[IDIOMA]["lat_lon"], True, COLOR_CAJA_DATOS_TEXTO) # Actualizado
    ...
    rumbo_etiqueta_surf = font.render(TEXTOS[IDIOMA]["rumbo"] + " :", True, COLOR_CAJA_DATOS_TEXTO) # Actualizado
    ...
    vel_etiqueta_surf = font.render(TEXTOS[IDIOMA]["velocidad"] + " :", True, COLOR_CAJA_DATOS_TEXTO) # Actualizado
    ...
    text_surface_titulo_cabeceo = font.render(TEXTOS[IDIOMA]["actitud"], True, COLOR_CAJA_DATOS_TEXTO) # Actualizado
    ...
    pitch_etiqueta_str = TEXTOS[IDIOMA]["pitch"] + " :" # Actualizado
    pitch_valor_str = "N/A"
    if att_pitch_str != "N/A":
    ...
    roll_etiqueta_str = TEXTOS[IDIOMA]["roll"] + "  :" # Actualizado
    roll_valor_display_str = "N/A"; roll_direccion_str = ""     
    if att_roll_str != "N/A":
        try:
            roll_val = float(att_roll_str); roll_valor_display_str = f"{roll_val:+.0f}°" 
            if roll_val > 0.1: roll_direccion_str = "ESTRIBOR" # Mantener o traducir si es necesario
            elif roll_val < -0.1: roll_direccion_str = "BABOR" # Mantener o traducir si es necesario
        except ValueError: pass 
    ...
    # --- Mensajes de Estado ---
    # print(f"DEBUG: Antes de checkear UMBRAL_SIN_DATOS_MS - nmea_data_stale: {nmea_data_stale}")
    if serial_port_available and ser and ser.is_open:
        ...
        # No hay 'else' aquí para nmea_data_stale; se resetea en las funciones de parseo.
            
    elif not serial_port_available: 
        # print(f"DEBUG: Puerto no disponible - nmea_data_stale: {nmea_data_stale}")
        if not nmea_data_stale: 
            reset_ui_data()
            nmea_data_stale = True 
    ...
    if mostrar_ventana_config_serial:
        ...
        titulo_surf = font.render(TEXTOS[IDIOMA]["titulo_config"], True, COLOR_TEXTO_NORMAL) # Actualizado
        ...
        cerrar_text = font.render("X", True, COLOR_TEXTO_NORMAL) # "X" no necesita traducción
        ...
        label_puerto_surf = font.render(TEXTOS[IDIOMA]["etiqueta_puerto"], True, COLOR_TEXTO_NORMAL) # Actualizado
        ...
        label_baudios_surf = font.render(TEXTOS[IDIOMA]["etiqueta_baudios"], True, COLOR_TEXTO_NORMAL) # Actualizado
        ...
        guardar_surf = font.render(TEXTOS[IDIOMA]["boton_guardar"] + " & Aplicar", True, COLOR_TEXTO_NORMAL) # Actualizado y modificado
        ...
        titulo_alarma_surf = font.render(TEXTOS[IDIOMA]["titulo_alarma"], True, COLOR_TEXTO_NORMAL) # Actualizado
        ...
        label_pitch_surf = font.render(TEXTOS[IDIOMA]["pitch_rango"], True, COLOR_TEXTO_NORMAL) # Actualizado
        ...
        label_roll_surf = font.render(TEXTOS[IDIOMA]["roll_rango"], True, COLOR_TEXTO_NORMAL) # Actualizado
        ...
        guardar_alarma_surf = font.render(TEXTOS[IDIOMA]["boton_guardar"], True, COLOR_TEXTO_NORMAL) # Actualizado
        ...
        salir_alarma_surf = font.render(TEXTOS[IDIOMA]["boton_salir"], True, COLOR_TEXTO_NORMAL) # Actualizado
        ...
        titulo_acerca_surf = font.render(TEXTOS[IDIOMA]["titulo_acerca"], True, COLOR_TEXTO_NORMAL) # Actualizado
        ...
        # Los textos de información de "Acerca de" pueden permanecer hardcodeados o agregarse al diccionario TEXTOS si se desea traducción.
        # Por simplicidad, los dejaremos como estaban por ahora.
        ...
        cerrar_acerca_surf = font.render(TEXTOS[IDIOMA]["boton_cerrar"], True, COLOR_TEXTO_NORMAL) # Actualizado
        screen.blit(cerrar_acerca_surf, cerrar_acerca_surf.get_rect(center=rect_boton_cerrar_acerca_de.center))
        
    # --- Dibujar Ventana de Idioma ---
    if mostrar_ventana_idioma:
        pygame.draw.rect(screen, COLOR_VENTANA_FONDO, rect_ventana_idioma)
        pygame.draw.rect(screen, COLOR_BORDE_VENTANA, rect_ventana_idioma, 2)
        ...
        rect_boton_es = pygame.Rect(rect_ventana_idioma.centerx - button_width_lang // 2, 
                                    rect_ventana_idioma.top + titulo_idioma_surf.get_height() + 30, 
                                    button_width_lang, button_height_lang)
        rect_boton_en = pygame.Rect(rect_ventana_idioma.centerx - button_width_lang // 2, 
                                    rect_boton_es.bottom + padding_y_lang, 
                                    button_width_lang, button_height_lang)

        # Botón Español
        pygame.draw.rect(screen, COLOR_BOTON_FONDO_3D if IDIOMA == "es" else COLOR_BOTON_FONDO, rect_boton_es)
        pygame.draw.rect(screen, COLOR_BOTON_BORDE, rect_boton_es, 1)
        ...
        # Botón Inglés
        pygame.draw.rect(screen, COLOR_BOTON_FONDO_3D if IDIOMA == "en" else COLOR_BOTON_FONDO, rect_boton_en)
        pygame.draw.rect(screen, COLOR_BOTON_BORDE, rect_boton_en, 1)
