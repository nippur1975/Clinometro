...
def cargar_configuracion_alarma():
    ...
    try:
        ...
        pass # Añadido pass para corregir IndentationError
...
# Definir dimensiones de la pantalla ANTES de usarlas para centrar la ventana
dimensiones = [1050, 430] 

# Dimensiones de la ventana de configuración
ventana_config_width = 400
ventana_config_height = 300 # Reducida para mejor ajuste centrado
...
COLOR_VENTANA_FONDO = (144, 238, 144) # Verde claro
COLOR_TEXTO_NORMAL = NEGRO
COLOR_BORDE_VENTANA = (170, 170, 170) # Restaurado para ventanas modales no 3D
# Colores para efecto 3D del borde de la ventana de config puerto
...
COLOR_BOTON_FONDO = (225, 225, 225) # Restaurado valor original
COLOR_BOTON_BORDE = (150, 150, 150) # Restaurado valor original

# Colores específicos para botones con efecto 3D (usados en ventana config puerto)
COLOR_BOTON_FONDO_3D = (210, 210, 210) 
COLOR_BOTON_BORDE_CLARO_3D = (230, 230, 230)
COLOR_BOTON_BORDE_OSCURO_3D = (130, 130, 130)

# Colores para inputs con efecto 3D
COLOR_INPUT_FONDO = BLANCO
COLOR_INPUT_BORDE_CLARO = (200, 200, 200) # Invertido para efecto inset
COLOR_INPUT_BORDE_OSCURO = (80, 80, 80)   # Invertido para efecto inset
...
# Ajustar posiciones de elementos internos a la nueva rect_ventana_config y altura
input_elements_top_offset = 55
input_elements_height = 30
label_width_config = 80 # Ancho para etiquetas "Puerto:", "Baudios:"
input_width_config = rect_ventana_config.width - label_width_config - 20 - 20 - 10 # Ajustar ancho del input

rect_input_puerto_config = pygame.Rect(rect_ventana_config.left + label_width_config + 20, rect_ventana_config.top + input_elements_top_offset, input_width_config, input_elements_height)
rect_lista_puertos_desplegable = None 
lista_rects_items_puerto = [] 

rect_input_baudios_display_config = pygame.Rect(rect_ventana_config.left + label_width_config + 20, rect_ventana_config.top + input_elements_top_offset + input_elements_height + 15, input_width_config, input_elements_height)
rect_lista_baudios_desplegable = None
lista_rects_items_baudios = []

button_config_width = 150
button_config_height = 40
rect_boton_guardar_config = pygame.Rect(rect_ventana_config.centerx - button_config_width // 2, rect_ventana_config.bottom - button_config_height - 20, button_config_width, button_config_height)
...
try: 
    ...
    print(f"An unexpected error occurred opening serial port: {e}") # Conservar este print de error inesperado
    serial_port_available = False
...
try:
    ...
    print(f"Error al cargar la imagen de fondo '{ruta_imagen_fondo}': {e}") # Conservar este print de error de carga
...
try:
    ...
    print(f"Error al cargar o escalar la imagen 'pitch.png': {e}") # Conservar este print de error de carga
    pitch_image_base_grande = None
...
try:
    ...
    print(f"Error al cargar o escalar la imagen 'roll.png': {e}") # Conservar este print de error de carga
    roll_image_base_grande = None
...
while not hecho:
                ...
                if not mostrar_ventana_config_serial and not mostrar_ventana_acerca_de and not mostrar_ventana_alarma: 
                    if rects_opciones_menu_barra and len(rects_opciones_menu_barra) > 0 and rects_opciones_menu_barra[0].collidepoint(evento.pos): 
                        ...
                        try:
                            ...
                            # print(f"Error al detectar puertos COM: {e_com}"); # Comentado
                    ...
                    if puerto_dropdown_activo:
                        ...
                        try:
                            ...
                            print(f"Error inesperado reabriendo puerto: {e}"); serial_port_available = False # Conservar este print
    ...
    if not serial_port_available and not mostrar_ventana_config_serial and not mostrar_ventana_alarma and not mostrar_ventana_acerca_de: 
        ...
        if ahora - ultimo_intento_reconeccion_tiempo > INTERVALO_RECONECCION_MS:
            ...
            try:
                ...
                # print(f"Fallo al reconectar: {e_reconect}"); # Comentado
                ser = None; serial_port_available = False 
            except Exception as e_reconect_general: 
                print(f"Error inesperado reconexión: {e_reconect_general}"); ser = None; serial_port_available = False # Conservar
    ...
    if serial_port_available and ser and ser.is_open: 
        try:
            ...
            print(f"SerialException lectura: {se}. Puerto desconectado."); ser.close(); ser = None; serial_port_available = False # Conservar
            ultimo_intento_reconeccion_tiempo = pygame.time.get_ticks() 
        except Exception as e:
            # print(f"Error parseo datos seriales: {e}") # Comentado
            pass
    ...
    if att_roll_str != "N/A" and valores_alarma: # Solo si tenemos valor de roll y configuración de alarma
        try:
            ...
            # print(f"Error al procesar valores de alarma o roll: {e}") # Comentado
    ...
    if att_pitch_str != "N/A" and valores_alarma: 
        try:
            ...
            # print(f"Error al procesar valores de alarma o pitch: {e}") # Comentado
            alarma_pitch_sentado_activa = False
            alarma_pitch_encabuzado_activa = False

# Lógica para enviar datos a ThingSpeak y CSV cada INTERVALO_ENVIO_DATOS_S
# La restauración de prints para esta sección se maneja arriba, donde se llama a guardar_csv y enviar_thingspeak.
# Esta sección duplicada y comentada puede eliminarse o mantenerse comentada.
# Por coherencia, si la de arriba está activa, esta debería estarlo o eliminarse.
# Dado que la anterior ya fue restaurada, esta se puede dejar comentada o eliminar.
# Para evitar confusión, me aseguro que la lógica de envío de datos solo esté activa en un lugar.
# La sección de arriba (alrededor de la línea 730-740) es la que se ejecuta.
    ...
    # print(f"DEBUG GESTOR: ahora={ahora:.2f}") 
    ...
    # print(f"DEBUG GESTOR: Condiciones activas detectadas: {len(alarmas_condiciones_activas_info)}") 
    # for al_info_debug in alarmas_condiciones_activas_info: 
    #    print(f"  DEBUG ACTIVA DETECTADA: {al_info_debug['id']}") 
    ...
    for alarma_info in alarmas_condiciones_activas_info:
        if ahora - globals()[alarma_info['tiempo_var_name']] >= alarma_info['intervalo']:
            ...
            # print(f"  DEBUG ALARMA LISTA PARA SONAR: {alarma_info['id']} (Ultima reprod: {globals()[alarma_info['tiempo_var_name']]:.2f}, Intervalo: {alarma_info['intervalo']})") 
    # print(f"DEBUG GESTOR: Alarmas listas para sonar: {len(alarmas_listas_para_sonar_info)}") 
    ...
    if not alarmas_condiciones_activas_info: 
        ...
        # print(f"DEBUG GESTOR: sonido_actual_terminado_o_pausa_cumplida = {sonido_actual_ha_terminado_o_pausa_cumplida}") 
    ...
    # --- DIBUJADO PRINCIPAL DE LA PANTALLA (FUERA DE LAS CONDICIONES DE VENTANAS MODALES) ---
    # Esta sección ahora se ejecutará en cada frame, antes de dibujar las ventanas modales (si alguna está activa).
    
    # Dibujar los círculos y sus contenidos
    ...
    # Mensajes de estado (NO HAY DATOS / Puerto desconectado)
    if serial_port_available and ser and ser.is_open:
        ahora = pygame.time.get_ticks()
    ...
    # --- DIBUJADO DE VENTANAS MODALES (SI ESTÁN ACTIVAS) ---
    # Estas se dibujarán encima de los elementos principales.
    if mostrar_ventana_config_serial:
        # Fondo de la ventana
        pygame.draw.rect(screen, COLOR_VENTANA_FONDO, rect_ventana_config)
        
        # Borde 3D para la ventana principal
        pygame.draw.line(screen, COLOR_BORDE_VENTANA_CLARO, rect_ventana_config.topleft, rect_ventana_config.topright, 2) # Arriba
        pygame.draw.line(screen, COLOR_BORDE_VENTANA_CLARO, rect_ventana_config.topleft, rect_ventana_config.bottomleft, 2) # Izquierda
        pygame.draw.line(screen, COLOR_BORDE_VENTANA_OSCURO, rect_ventana_config.bottomleft, rect_ventana_config.bottomright, 2) # Abajo
        pygame.draw.line(screen, COLOR_BORDE_VENTANA_OSCURO, rect_ventana_config.topright, rect_ventana_config.bottomright, 2) # Derecha

        # Título
        titulo_surf = font.render("Configuración Puerto Serie", True, COLOR_TEXTO_NORMAL)
        screen.blit(titulo_surf, (rect_ventana_config.centerx - titulo_surf.get_width() // 2, rect_ventana_config.top + 15))

        # Botón Cerrar (X) con efecto 3D
        pygame.draw.rect(screen, COLOR_BOTON_FONDO_3D, rect_boton_cerrar_config) # Usar FONDO_3D
        ...
        # Input Puerto con efecto 3D (inset)
        pygame.draw.rect(screen, COLOR_INPUT_FONDO, rect_input_puerto_config, 0) 
        pygame.draw.line(screen, COLOR_INPUT_BORDE_OSCURO, rect_input_puerto_config.topleft, rect_input_puerto_config.topright, 1)
        pygame.draw.line(screen, COLOR_INPUT_BORDE_OSCURO, rect_input_puerto_config.topleft, rect_input_puerto_config.bottomleft, 1)
        pygame.draw.line(screen, COLOR_INPUT_BORDE_CLARO, rect_input_puerto_config.bottomleft, rect_input_puerto_config.bottomright, 1)
        pygame.draw.line(screen, COLOR_INPUT_BORDE_CLARO, rect_input_puerto_config.topright, rect_input_puerto_config.bottomright, 1)
        input_puerto_surf = font.render(input_puerto_str, True, COLOR_TEXTO_NORMAL)
        screen.blit(input_puerto_surf, (rect_input_puerto_config.left + 5, rect_input_puerto_config.centery - input_puerto_surf.get_height() // 2))
        pygame.draw.polygon(screen, COLOR_TEXTO_NORMAL, [(rect_input_puerto_config.right - 15, rect_input_puerto_config.centery - 3),(rect_input_puerto_config.right - 5, rect_input_puerto_config.centery - 3),(rect_input_puerto_config.right - 10, rect_input_puerto_config.centery + 3)])
        ...
        if puerto_dropdown_activo:
            lista_rects_items_puerto.clear()
            item_height = input_puerto_surf.get_height() + 4 
            dropdown_height = min(item_height * len(lista_puertos_detectados), rect_ventana_config.height - (rect_input_puerto_config.bottom - rect_ventana_config.top) - 10 - (rect_input_baudios_display_config.height + 15) - (rect_boton_guardar_config.height + 20)) # Espacio para otros elementos
            rect_lista_puertos_desplegable = pygame.Rect(rect_input_puerto_config.left, rect_input_puerto_config.bottom, rect_input_puerto_config.width, dropdown_height)
            pygame.draw.rect(screen, COLOR_DROPDOWN_FONDO, rect_lista_puertos_desplegable) 
            pygame.draw.rect(screen, COLOR_DROPDOWN_BORDE, rect_lista_puertos_desplegable, 1) 
            for i, port_name in enumerate(lista_puertos_detectados):
        ...
        # Input Baudios con efecto 3D (inset)
        pygame.draw.rect(screen, COLOR_INPUT_FONDO, rect_input_baudios_display_config, 0) 
        pygame.draw.line(screen, COLOR_INPUT_BORDE_OSCURO, rect_input_baudios_display_config.topleft, rect_input_baudios_display_config.topright, 1)
        pygame.draw.line(screen, COLOR_INPUT_BORDE_OSCURO, rect_input_baudios_display_config.topleft, rect_input_baudios_display_config.bottomleft, 1)
        pygame.draw.line(screen, COLOR_INPUT_BORDE_CLARO, rect_input_baudios_display_config.bottomleft, rect_input_baudios_display_config.bottomright, 1)
        pygame.draw.line(screen, COLOR_INPUT_BORDE_CLARO, rect_input_baudios_display_config.topright, rect_input_baudios_display_config.bottomright, 1)
        baudios_surf = font.render(str(lista_baudios_seleccionables[input_baudios_idx]), True, COLOR_TEXTO_NORMAL)
        screen.blit(baudios_surf, baudios_surf.get_rect(center=rect_input_baudios_display_config.center))
        pygame.draw.polygon(screen, COLOR_TEXTO_NORMAL, [(rect_input_baudios_display_config.right - 15, rect_input_baudios_display_config.centery - 3),(rect_input_baudios_display_config.right - 5, rect_input_baudios_display_config.centery - 3),(rect_input_baudios_display_config.right - 10, rect_input_baudios_display_config.centery + 3)])
        ...
        if baudios_dropdown_activo:
            lista_rects_items_baudios.clear()
            item_height = baudios_surf.get_height() + 4
            dropdown_height = min(item_height * len(lista_baudios_seleccionables), rect_ventana_config.height - (rect_input_baudios_display_config.bottom - rect_ventana_config.top) - 10 - (rect_boton_guardar_config.height + 20)) # Espacio para el botón
            rect_lista_baudios_desplegable = pygame.Rect(rect_input_baudios_display_config.left, rect_input_baudios_display_config.bottom, rect_input_baudios_display_config.width, dropdown_height)
            pygame.draw.rect(screen, COLOR_DROPDOWN_FONDO, rect_lista_baudios_desplegable)
            pygame.draw.rect(screen, COLOR_DROPDOWN_BORDE, rect_lista_baudios_desplegable, 1)
            for i, baud_rate in enumerate(lista_baudios_seleccionables):
        ...
        pygame.draw.rect(screen, COLOR_BOTON_FONDO_3D, rect_boton_guardar_config) # Usar FONDO_3D
        # Borde claro arriba e izquierda
        ...
        # Usar el color de fondo original de las ventanas modales, no el verde claro de config. puerto
        ...
        rect_boton_guardar_alarma = pygame.Rect(rect_ventana_alarma.centerx - button_width - 10, rect_ventana_alarma.bottom - 60, button_width, button_height)
        rect_boton_salir_alarma = pygame.Rect(rect_ventana_alarma.centerx + 10, rect_ventana_alarma.bottom - 60, button_width, button_height)
        pygame.draw.rect(screen, COLOR_BOTON_FONDO, rect_boton_guardar_alarma) # Usar color de botón normal
        pygame.draw.rect(screen, COLOR_BOTON_BORDE, rect_boton_guardar_alarma, 1) # Usar borde de botón normal
        guardar_alarma_surf = font.render("Guardar", True, COLOR_TEXTO_NORMAL)
        screen.blit(guardar_alarma_surf, guardar_alarma_surf.get_rect(center=rect_boton_guardar_alarma.center))
        pygame.draw.rect(screen, COLOR_BOTON_FONDO, rect_boton_salir_alarma) # Usar color de botón normal
        pygame.draw.rect(screen, COLOR_BOTON_BORDE, rect_boton_salir_alarma, 1) # Usar borde de botón normal
        salir_alarma_surf = font.render("Salir", True, COLOR_TEXTO_NORMAL)
        screen.blit(salir_alarma_surf, salir_alarma_surf.get_rect(center=rect_boton_salir_alarma.center))

    elif mostrar_ventana_acerca_de:
        rect_ventana_acerca_de = pygame.Rect(250, 150, 400, 250) 
        # Usar el color de fondo original de las ventanas modales
        ...
        pygame.draw.rect(screen, COLOR_BOTON_FONDO, rect_boton_cerrar_acerca_de) # Usar color de botón normal
        pygame.draw.rect(screen, COLOR_BOTON_BORDE, rect_boton_cerrar_acerca_de, 1) # Usar borde de botón normal
    ...
    else: # Esta es la cláusula ELSE principal que dibuja la pantalla cuando ninguna ventana modal está activa.
        # Dibujar el fondo y los elementos principales aquí.
        # Esto asegura que siempre se dibujen a menos que una ventana modal esté activa Y explícitamente decida no dibujarlos.
        # (Lo cual haremos en el siguiente paso del plan).
