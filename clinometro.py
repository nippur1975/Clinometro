...
CALA_MANIOBRA_MIN_TURN_DEG = 270.0
CALA_MANIOBRA_MIN_DURATION = 300.0  # 5 min
CALA_MANIOBRA_MAX_DURATION = 1200.0 # 20 min

CALA_STOP_SPEED_THRESHOLD = 2.0     # Subido a 2.0 para detectar la parada antes
CALA_VALIDATION_DURATION = 600.0    # 10 minutos de validación
CALA_STEAMING_SPEED_THRESHOLD = 4.0
CALA_CONFIRMATION_DURATION = 300.0

# Criterios de Validación (Huella Digital RELAJADA para mar real)
CALA_VAL_MAX_SPEED = 1.8            # Subido a 1.8 (tolera picos por oleaje)
CALA_VAL_AVG_SPEED_MIN = 0.1        # Bajado a 0.1 (a veces quedan muy quietos)
CALA_VAL_AVG_SPEED_MAX = 1.0        # Subido a 1.0 (para aceptar el promedio de 0.69 del log)
CALA_VAL_DRIFT_MAX_M = 400.0        # Subido a 400m (más margen de deriva)
...
# Nuevas variables globales para el estado de la última alarma y cala
current_alarm_info = {
    "TipoAlarma": None,
    "EstadoAlarma": None,
    "ValorActual": None,
...
def reset_ui_data():
    ...
    global tiempo_inicio_condicion_roll_babor, tiempo_inicio_condicion_roll_estribor
    global tiempo_inicio_condicion_pitch_sentado, tiempo_inicio_condicion_pitch_encabuzado
    ...
    # Resetear temporizadores de condición de alarma (persistencia)
    ...
    try:
        ...
        with open(CSV_FILENAME, 'a', newline='') as f: 
            if not file_exists or is_empty:
                writer = csv.writer(f)
                writer.writerow(["ShipID", "Date_event", "Pitch", "Roll", "Latitud", "Longitud", "Velocidad", "Rumbo", "ROT", "Switch1", "Switch2", "TipoAlarma", "EstadoAlarma", "ValorActual", "UmbralConfigurado", "CalaID"])
...
def get_daily_cala_count(): 
    ...
    # Reset diario 
    if last_cala_date != current_date: 
        daily_cala_count = 0 
...
def registrar_evento_cala(ship_id, date_event, cala_id_num, lat, lon):
    ...
    enviar_y_guardar_datos_periodicamente(force=True, overrides=overrides)

def guardar_alarma_csv(timestamp, tipo_alarma, estado_alarma, valor_actual, umbral_configurado):
    """
    Actualiza el estado global de alarmas y fuerza el guardado inmediato.
    """
    current_alarm_info.update({
        "TipoAlarma": tipo_alarma,
        "EstadoAlarma": estado_alarma,
        "ValorActual": valor_actual,
        "UmbralConfigurado": umbral_configurado
    })
    print(f"INFO: Registrando evento de alarma: {tipo_alarma} - {estado_alarma}")
    # Forzar envío y guardado inmediato a Azure/CSV
    enviar_y_guardar_datos_periodicamente(force=True)

def procesar_logica_alarmas():
    """
    Verifica los ángulos, gestiona temporizadores y dispara alarmas.
    Funciona tanto con ventana visible como oculta.
    """
    global alarma_roll_babor_activa, alarma_roll_estribor_activa
    global alarma_pitch_sentado_activa, alarma_pitch_encabuzado_activa
    global tiempo_inicio_condicion_roll_babor, tiempo_inicio_condicion_roll_estribor
    ...
    if not valores_alarma: return
    ahora = time.time()

    try:
        # Leer umbrales
        umbral_min_roll = float(valores_alarma["min_roll_neg"])
        umbral_max_roll = float(valores_alarma["max_roll_pos"])
        umbral_min_pitch = float(valores_alarma["min_pitch_neg"])
        umbral_max_pitch = float(valores_alarma["max_pitch_pos"])

        # --- ROLL BABOR ---
        if (ts_roll_float > umbral_max_roll if att_roll_str != "N/A" else False):
            if tiempo_inicio_condicion_roll_babor == 0.0: tiempo_inicio_condicion_roll_babor = ahora
            if ahora - tiempo_inicio_condicion_roll_babor >= DURACION_ACTIVACION_ALARMA and not alarma_roll_babor_activa:
                alarma_roll_babor_activa = True
                guardar_alarma_csv(ts_timestamp_str, "ROLL_BABOR", "ACTIVADA", ts_roll_float, umbral_max_roll)
        else:
            tiempo_inicio_condicion_roll_babor = 0.0
            if alarma_roll_babor_activa:
                alarma_roll_babor_activa = False
                guardar_alarma_csv(ts_timestamp_str, "ROLL_BABOR", "DESACTIVADA", ts_roll_float, umbral_max_roll)

        # --- ROLL ESTRIBOR ---
        if (ts_roll_float < umbral_min_roll if att_roll_str != "N/A" else False):
            if tiempo_inicio_condicion_roll_estribor == 0.0: tiempo_inicio_condicion_roll_estribor = ahora
            if ahora - tiempo_inicio_condicion_roll_estribor >= DURACION_ACTIVACION_ALARMA and not alarma_roll_estribor_activa:
                alarma_roll_estribor_activa = True
                guardar_alarma_csv(ts_timestamp_str, "ROLL_ESTRIBOR", "ACTIVADA", ts_roll_float, umbral_min_roll)
        else:
            tiempo_inicio_condicion_roll_estribor = 0.0
            if alarma_roll_estribor_activa:
                alarma_roll_estribor_activa = False
                guardar_alarma_csv(ts_timestamp_str, "ROLL_ESTRIBOR", "DESACTIVADA", ts_roll_float, umbral_min_roll)

        # --- PITCH ENCABUZADO ---
        if (ts_pitch_float < umbral_min_pitch if att_pitch_str != "N/A" else False):
            if tiempo_inicio_condicion_pitch_encabuzado == 0.0: tiempo_inicio_condicion_pitch_encabuzado = ahora
            if ahora - tiempo_inicio_condicion_pitch_encabuzado >= DURACION_ACTIVACION_ALARMA and not alarma_pitch_encabuzado_activa:
                alarma_pitch_encabuzado_activa = True
                guardar_alarma_csv(ts_timestamp_str, "PITCH_ENCABUZADO", "ACTIVADA", ts_pitch_float, umbral_min_pitch)
        else:
            tiempo_inicio_condicion_pitch_encabuzado = 0.0
            if alarma_pitch_encabuzado_activa:
                alarma_pitch_encabuzado_activa = False
                guardar_alarma_csv(ts_timestamp_str, "PITCH_ENCABUZADO", "DESACTIVADA", ts_pitch_float, umbral_min_pitch)

        # --- PITCH SENTADO ---
        if (ts_pitch_float > umbral_max_pitch if att_pitch_str != "N/A" else False):
            if tiempo_inicio_condicion_pitch_sentado == 0.0: tiempo_inicio_condicion_pitch_sentado = ahora
            if ahora - tiempo_inicio_condicion_pitch_sentado >= DURACION_ACTIVACION_ALARMA and not alarma_pitch_sentado_activa:
                alarma_pitch_sentado_activa = True
                guardar_alarma_csv(ts_timestamp_str, "PITCH_SENTADO", "ACTIVADA", ts_pitch_float, umbral_max_pitch)
        else:
            tiempo_inicio_condicion_pitch_sentado = 0.0
            if alarma_pitch_sentado_activa:
                alarma_pitch_sentado_activa = False
                guardar_alarma_csv(ts_timestamp_str, "PITCH_SENTADO", "DESACTIVADA", ts_pitch_float, umbral_max_pitch)
...
class DetectorCala: 
    def __init__(self): 
        self.state = 0  
        self.maniobra_inicio_ts = None 
        self.giro_acumulado = 0.0 
        self.last_heading = None 
        self.conteo_calas_dia = 0 
        self.fecha_actual = datetime.now(timezone.utc).date() 
         
        # Variables nuevas para la lógica mejorada 
        self.potential_start_ts = None 
        self.is_confirmed = False # Para saber si ya logueamos esta cala 
    ...
    def reset_to_search(self): 
        ...
        print("INFO: Detector reseteado a BÚSQUEDA.") 
 
    def procesar_dato(self, timestamp, velocidad, rumbo, lat, lon): 
        # Reset diario 
        ...
        # ESTADO 0: BÚSQUEDA (Acumular Giro) 
        if self.state == 0: 
            ...
            # Acumular solo si hay movimiento mínimo para evitar ruido de GPS parado 
            if velocidad > 1.0: 
                self.giro_acumulado += diff 
             
            self.last_heading = rumbo 
 
            # Si giró suficiente (> 270°), pasamos a esperar la parada 
            if abs(self.giro_acumulado) >= CALA_MANIOBRA_MIN_TURN_DEG: 
                if self.maniobra_inicio_ts is None: 
                    self.maniobra_inicio_ts = timestamp 
        ...
        # ESTADO 1: ESPERANDO PARADA 
            ...
            # Si la velocidad baja, entramos en "Posible Cala" (Validación) 
            if velocidad < CALA_STOP_SPEED_THRESHOLD: 
                print(f"INFO: Velocidad baja detectada ({velocidad} kn). Iniciando validación de tiempo...") 
                self.state = 2 
        ...
        # Aquí nos quedamos MIENTRAS el barco esté parado (pescando) 
            ...
            if not self.is_confirmed and duration_seconds >= CALA_CONFIRMATION_DURATION: 
                ...
                # Sincronizar conteo 
                conteo_archivo = get_daily_cala_count() 
                if conteo_archivo > self.conteo_calas_dia: 
                    self.conteo_calas_dia = conteo_archivo 
                self.conteo_calas_dia += 1 
    ...
    try:
        with open(CSV_FILENAME, 'a', newline='') as f: 
            ...
            writer.writerow([
                ...
                datos.get('Sentina1'),
                datos.get('Sentina2'),
                datos.get('TipoAlarma'),
                datos.get('EstadoAlarma'),
                datos.get('ValorActual'),
                datos.get('UmbralConfigurado'),
                datos.get('CalaID')
            ])
    except Exception as e:
...
def enviar_thingspeak(datos):
    # Esta función ahora solo prepara y lanza el hilo con una lista de un solo payload.
    estado_alarma_para_thingspeak = "SIN ALARMA"
    
    # Construir estado de alarma desde datos si es posible, o usar globales (compatibilidad)
    # Usaremos los datos pasados si tienen info de alarma (TipoAlarma != None)
    # Pero los flags booleanos globales son más expresivos para "estado actual" visual.
    # Para consistencia con el payload 'status' de texto:
    if datos.get('TipoAlarma'):
         estado_alarma_para_thingspeak = f"ALARMA {datos.get('TipoAlarma')}"
    else:
        if alarma_roll_babor_activa:
    ...
    payload = {
        ...
        'field1': datos.get('Pitch'),
        'field2': datos.get('Roll'),
        'field3': datos.get('Latitud'),
        'field4': datos.get('Longitud'),
        'field5': datos.get('Velocidad'),
        'field6': datos.get('Rumbo'),
        'field7': datos.get('Altitude', 0.0),
        'field8': datos.get('Rot'),
        'status': f"Sentina1: {datos.get('Sentina1')}, Sentina2: {datos.get('Sentina2')}, Alarma: {estado_alarma_para_thingspeak}"
    }
...
def worker_enviar_sql(lista_payloads):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            (ShipID, Date_event, Pitch, Roll, Latitud, Longitud, Velocidad, Rumbo, Rot, Sentina1, Sentina2, TipoAlarma, EstadoAlarma, ValorActual, UmbralConfigurado, CalaID) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """
            ...
            datos_para_insertar = [
                (p['ShipID'], p['Date_event'], p['Pitch'], p['Roll'], p['Latitud'], 
                 ...
                 p['TipoAlarma'], p['EstadoAlarma'], p['ValorActual'],
                 p['UmbralConfigurado'], p['CalaID'])
...
def enviar_y_guardar_datos_periodicamente(force=False, overrides=None):
    ...
    # Construir diccionario de datos base con valores globales
    timestamp_base = ts_timestamp_str if ts_timestamp_str != "N/A" else datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")

    datos = {
        'ShipID': API_KEY_GOOGLE_CLOUD if API_KEY_GOOGLE_CLOUD else "(no definido)",
        'Date_event': timestamp_base,
        'Pitch': ts_pitch_float,
        'Roll': ts_roll_float,
        'Latitud': ts_lat_decimal,
        'Longitud': ts_lon_decimal,
        'Velocidad': ts_speed_float,
        'Rumbo': ts_heading_float,
        'Rot': rot_float,
        'Altitude': ts_altitude_float,
        'Sentina1': switch1_status,
        'Sentina2': switch2_status,
        'TipoAlarma': current_alarm_info["TipoAlarma"],
        'EstadoAlarma': current_alarm_info["EstadoAlarma"],
        'ValorActual': current_alarm_info["ValorActual"],
        'UmbralConfigurado': current_alarm_info["UmbralConfigurado"],
        'CalaID': current_cala_id
    }

    # Aplicar overrides si existen (e.g., para eventos específicos de cala o alarma)
    if overrides:
        datos.update(overrides)

    # Evitar el envío de registros con todos los valores numéricos en cero (si no hay overrides que lo justifiquen)
    # Si hay overrides (ej. alarma o cala), forzamos el envío aunque sean ceros (aunque raramente lo serán)
    if not overrides and (datos['Pitch'] == 0.0 and datos['Roll'] == 0.0 and
        datos['Latitud'] == 0.0 and datos['Longitud'] == 0.0 and
        datos['Velocidad'] == 0.0 and datos['Rumbo'] == 0.0 and datos['Rot'] == 0.0):
    ...
    # Preparar los datos para el log en consola
    ...
    log_msg = (
        f"Destino: {destino_log}, ID:{datos['ShipID']}, Date:{datos['Date_event']}, "
        f"P:{datos['Pitch']:.1f}, R:{datos['Roll']:.1f}, "
        f"Lat:{datos['Latitud']}, Lon:{datos['Longitud']}, Spd:{datos['Velocidad']}, Hdg:{datos['Rumbo']}, "
        f"ROT:{datos['Rot']:.1f}, S1:{datos['Sentina1']}, S2:{datos['Sentina2']}"
    )
    if datos.get('CalaID'):
        log_msg += f", CalaID:{datos['CalaID']}"
    ...
    if network_available:
        if SERVICIO_DATOS_ACTUAL == "thingspeak":
            ...
            # worker_enviar_sql espera una lista de diccionarios con las claves correctas
            # 'datos' ya tiene las claves (ShipID, Date_event, etc.) coincidentes con lo que espera SQL
            # excepto 'Altitude' que sobra pero no molesta si no se usa en worker.

            # Crear payload para SQL (filtrando lo necesario si se quisiera, pero worker usa claves específicas)
            # worker usa: ShipID, Date_event, Pitch, Roll, Latitud, Longitud, Velocidad, Rumbo, Rot, Sentina1, Sentina2, TipoAlarma, EstadoAlarma, ValorActual, UmbralConfigurado, CalaID
            # Todas están en 'datos'.

            # Enviar el nuevo payload como una lista de un solo elemento
            sql_thread = threading.Thread(target=worker_enviar_sql, args=([datos],))
            sql_thread.daemon = True
            sql_thread.start()
...
def main():
    ...
    global tiempo_inicio_condicion_roll_babor, tiempo_inicio_condicion_roll_estribor, tiempo_inicio_condicion_pitch_sentado, tiempo_inicio_condicion_pitch_encabuzado
    ...
    # Variables para temporizadores de activación de alarma (1 minuto)
    tiempo_inicio_condicion_roll_babor = 0.0
    tiempo_inicio_condicion_roll_estribor = 0.0
    tiempo_inicio_condicion_pitch_sentado = 0.0
    tiempo_inicio_condicion_pitch_encabuzado = 0.0
    DURACION_ACTIVACION_ALARMA = 60.0 # segundos

    # Inicializar detector de calas
    detector_cala = DetectorCala()
    ultimo_tiempo_procesamiento_cala = 0.0
    ...
    while running:
        if window_visible:
            ...
            while not window_visible and running:
                ...
                # Lógica de detección de calas (si hay datos NMEA frescos)
                if not nmea_data_stale:
