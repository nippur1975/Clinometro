...
def show_window(_icon, _item): # _icon is the pystray.Icon instance
    global request_restore_display
    # This function is called from pystray's thread.
    # Only set the flag and stop the icon. Avoid Pygame calls from this thread.
    request_restore_display = True
    if _icon: # pystray passes the icon instance as the first argument
        _icon.stop()

def hide_window():
    global tray_icon, dimensiones
    # pygame.display.iconify() # No longer needed as we quit and restore
    pygame.display.quit() # Oculta y desinicializa la ventana principal de Pygame
    ...
    tray_icon.run()
    tray_icon = None # Clear the global reference after run() finishes
